...
<dialog class={styles.modal + (scrollable ? ` ${styles.scrollable}` : "")} bind:this={dialog} on:close={hide} on:click|self={hide}>
    <button class={[controlStyles.button, styles.closeButton].join(" ")} on:click={hide}>
        <Close />
    </button>
